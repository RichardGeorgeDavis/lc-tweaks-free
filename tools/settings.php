...
<div class="sett-wrap">
	...
	<div class="tool-wrap dlck-scope-rules-card">
		<div class="lc-kit">
			<div class="box-title">
				<h3><?php esc_html_e( 'Scope Rules', 'lc-tweaks' ); ?></h3>
				<div class="box-descr">
					<p><?php esc_html_e( 'Limit selected tweaks to specific URLs, login state, and user roles on frontend requests.', 'lc-tweaks' ); ?></p>
				</div>
			</div>
			<div class="box-content">
				<p>
					<label>
						...
						<?php esc_html_e( 'Enable scope rules', 'lc-tweaks' ); ?>
					</label>
				</p>
				<div id="dlck_scope_rules_options_wrap" class="dlck-scope-rules-options-wrap" <?php if ( '1' !== (string) $dlck_scope_rules_enabled ) : ?>style="display:none;"<?php endif; ?>>
					<p>
						<label for="dlck_scope_rules_options"><?php esc_html_e( 'Target option keys (one per line)', 'lc-tweaks' ); ?></label>
					...
					<p>
						<label for="dlck_scope_rules_logged_state"><?php esc_html_e( 'Apply only for', 'lc-tweaks' ); ?></label>
						<select name="dlck_scope_rules_logged_state" id="dlck_scope_rules_logged_state">
							<option value="all" <?php selected( 'all', $dlck_scope_rules_logged_state ); ?>><?php esc_html_e( 'All visitors', 'lc-tweaks' ); ?></option>
							<option value="logged_in" <?php selected( 'logged_in', $dlck_scope_rules_logged_state ); ?>><?php esc_html_e( 'Logged-in users only', 'lc-tweaks' ); ?></option>
							<option value="logged_out" <?php selected( 'logged_out', $dlck_scope_rules_logged_state ); ?>><?php esc_html_e( 'Logged-out users only', 'lc-tweaks' ); ?></option>
						</select>
					</p>
					<p>
						<label for="dlck_scope_rules_roles"><?php esc_html_e( 'Roles (one per line, optional)', 'lc-tweaks' ); ?></label>
					...
					<p>
						<label for="dlck_scope_rules_include_paths"><?php esc_html_e( 'Include paths (one per line, optional)', 'lc-tweaks' ); ?></label>
					...
					<p>
						<label for="dlck_scope_rules_exclude_paths"><?php esc_html_e( 'Exclude paths (one per line, optional)', 'lc-tweaks' ); ?></label>
					...
					<p class="description"><?php esc_html_e( 'Wildcard tips: use * as a path wildcard (for example /shop/*). Exclude paths take priority when both lists match.', 'lc-tweaks' ); ?></p>
					...
					<div class="dlck-scope-rules-tester">
						<h4><?php esc_html_e( 'Scope Rules Tester', 'lc-tweaks' ); ?></h4>
						<p class="description"><?php esc_html_e( 'Simulate a frontend request and check whether a target option would run.', 'lc-tweaks' ); ?></p>
						<p>
							<label for="dlck_scope_test_option"><?php esc_html_e( 'Option key', 'lc-tweaks' ); ?></label>
						...
						<p>
							<label for="dlck_scope_test_path"><?php esc_html_e( 'Request path', 'lc-tweaks' ); ?></label>
							<input type="text" id="dlck_scope_test_path" value="/" placeholder="/shop/sale-item" />
							<button type="button" class="button dlck-scope-path-helper" id="dlck_scope_test_use_current_path"><?php esc_html_e( 'Use Current Browser Path', 'lc-tweaks' ); ?></button>
						</p>
						<p>
							<label for="dlck_scope_test_user_state"><?php esc_html_e( 'User state', 'lc-tweaks' ); ?></label>
							<select id="dlck_scope_test_user_state">
								<option value="logged_out"><?php esc_html_e( 'Logged out', 'lc-tweaks' ); ?></option>
								<option value="logged_in"><?php esc_html_e( 'Logged in', 'lc-tweaks' ); ?></option>
							</select>
						</p>
						<p>
							<label for="dlck_scope_test_roles"><?php esc_html_e( 'User roles (optional, one per line)', 'lc-tweaks' ); ?></label>
							<textarea id="dlck_scope_test_roles" rows="3" cols="60" placeholder="administrator&#10;editor"></textarea>
						</p>
						<p>
							<button type="button" class="dlck-settings-button" id="dlck_scope_test_run"><?php esc_html_e( 'Run Scope Test', 'lc-tweaks' ); ?></button>
						</p>
	...
	<div class="tool-wrap">
		<div class="lc-kit">
			<div class="box-title">
				<h3><?php esc_html_e( 'Multisite Policy Mode', 'lc-tweaks' ); ?></h3>
				<div class="box-descr">
					<p><?php esc_html_e( 'Set network defaults for LC Tweaks and optionally allow per-site overrides.', 'lc-tweaks' ); ?></p>
				</div>
			</div>
			<div class="box-content">
				<p>
					<strong><?php esc_html_e( 'Current status:', 'lc-tweaks' ); ?></strong>
					<?php
					if ( $dlck_multisite_policy_enabled === '1' ) {
						echo esc_html__( 'Enabled', 'lc-tweaks' );
						echo ' | ';
						echo ( $dlck_multisite_allow_site_overrides === '1' )
							? esc_html__( 'Per-site overrides allowed', 'lc-tweaks' )
							: esc_html__( 'Per-site overrides locked', 'lc-tweaks' );
					} else {
						echo esc_html__( 'Disabled', 'lc-tweaks' );
					}
					?>
				</p>
				<p>
					...
					echo esc_html(
						sprintf(
							__( 'Network defaults: %1$d keys | Source site ID: %2$s', 'lc-tweaks' ),
				...
				<?php if ( $dlck_multisite_policy_updated > 0 ) : ?>
					<p class="description">
						...
						echo esc_html( sprintf( __( 'Last policy update: %s', 'lc-tweaks' ), wp_date( 'Y-m-d H:i:s', $dlck_multisite_policy_updated ) ) );
						?>
					</p>
				<?php endif; ?>
				...
				<?php if ( $dlck_can_manage_network_policy ) : ?>
					<p>
						<label>
							...
							<?php esc_html_e( 'Enable network policy mode', 'lc-tweaks' ); ?>
						</label>
					</p>
					<p>
						<label>
							...
							<?php esc_html_e( 'Allow per-site overrides', 'lc-tweaks' ); ?>
						</label>
					</p>
					<p>
						<label>
							<input type="checkbox" name="dlck_network_policy_sync_defaults" value="1" />
							<?php esc_html_e( 'Sync network defaults from this site\'s current settings', 'lc-tweaks' ); ?>
						</label>
					</p>
					<p>
						<label>
							<input type="checkbox" name="dlck_network_policy_clear_local_overrides" value="1" />
							<?php esc_html_e( 'Clear this site\'s local overrides after saving policy', 'lc-tweaks' ); ?>
						</label>
					</p>
					<p class="description"><?php esc_html_e( 'Tip: keep "Allow per-site overrides" off to enforce network defaults on all sites.', 'lc-tweaks' ); ?></p>
					...
					<?php submit_button( __( 'Save Multisite Policy', 'lc-tweaks' ), 'dlck-settings-button', 'dlck_network_policy_submit', false, array( 'id' => 'dlck_network_policy_submit' ) ); ?>
				<?php else : ?>
					<p class="description"><?php esc_html_e( 'Only network administrators can modify this policy. Site admins can view the active status above.', 'lc-tweaks' ); ?></p>
				<?php endif; ?>
			</div>
		</div>
	</div>
	...
	<div class="tool-wrap">
		<div class="lc-kit">
			<div class="box-title">
				<h3><?php esc_html_e( 'Presets', 'lc-tweaks' ); ?></h3>
				<div class="box-descr">
					<p><?php esc_html_e( 'Apply curated bundles of settings for common site goals. Only preset-managed options are updated.', 'lc-tweaks' ); ?></p>
				</div>
			</div>
			<div class="box-content">
				<?php if ( empty( $dlck_settings_presets ) ) : ?>
					<p><?php esc_html_e( 'No presets are currently available.', 'lc-tweaks' ); ?></p>
				<?php else : ?>
					<p>
						<label for="dlck_preset_key"><?php esc_html_e( 'Choose preset', 'lc-tweaks' ); ?></label>
						<select name="dlck_preset_key" id="dlck_preset_key">
							<option value="" <?php selected( '', $dlck_selected_preset_key ); ?>><?php esc_html_e( 'Select a preset', 'lc-tweaks' ); ?></option>
					...
					?>
					<?php submit_button( __( 'Apply Preset', 'lc-tweaks' ), 'dlck-settings-button', 'dlck_apply_preset_submit', false, $dlck_apply_preset_button_args ); ?>
					<?php if ( $dlck_has_preset_restore_payload ) : ?>
						...
						if ( $dlck_preset_restore_label !== '' ) {
							...
							$dlck_restore_info_parts[] = sprintf( __( 'last preset: %s', 'lc-tweaks' ), $dlck_preset_restore_label );
						}
						if ( $dlck_preset_restore_created > 0 ) {
							/* translators: %s: date/time */
							$dlck_restore_info_parts[] = sprintf( __( 'backup created: %s', 'lc-tweaks' ), wp_date( 'Y-m-d H:i:s', $dlck_preset_restore_created ) );
						}
						?>
						...
						<?php submit_button( __( 'Restore Previous Settings', 'lc-tweaks' ), 'dlck-settings-button', 'dlck_restore_preset_submit', false, array( 'id' => 'dlck_restore_preset_submit' ) ); ?>
					<?php endif; ?>
				<?php endif; ?>
			</div>
		</div>
	...
	<div class="tool-wrap">
		<div class="lc-kit">
			<div class="box-title">
				<h3><?php esc_html_e( 'Preflight Check', 'lc-tweaks' ); ?></h3>
				<div class="box-descr">
					<p><?php esc_html_e( 'Quick conflict scan for settings that depend on each other or are mutually exclusive.', 'lc-tweaks' ); ?></p>
				</div>
			</div>
			<div class="box-content">
				<?php if ( empty( $dlck_preflight_conflicts ) ) : ?>
					<p><?php esc_html_e( 'No conflicts detected in current settings.', 'lc-tweaks' ); ?></p>
				<?php else : ?>
	...
	<div class="tool-wrap">
		<div class="lc-kit">
			<div class="box-title">
				<h3><?php esc_html_e( 'Snapshots & Rollback', 'lc-tweaks' ); ?></h3>
				<div class="box-descr">
					<p><?php esc_html_e( 'LC Tweaks stores up to 5 snapshots of previous settings so you can quickly roll back after changes.', 'lc-tweaks' ); ?></p>
				</div>
			</div>
			<div class="box-content">
				<?php if ( empty( $dlck_settings_snapshots ) ) : ?>
					<p><?php esc_html_e( 'No snapshots available yet. Save settings once to create the first snapshot.', 'lc-tweaks' ); ?></p>
				<?php else : ?>
					<p><?php esc_html_e( 'Latest 5 snapshots are kept automatically.', 'lc-tweaks' ); ?></p>
					<select name="dlck_snapshot_id" id="dlck_snapshot_id">
						<?php foreach ( $dlck_settings_snapshots as $index => $snapshot ) : ?>
							...
							$user_login    = ! empty( $snapshot['user_login'] ) ? (string) $snapshot['user_login'] : __( 'unknown user', 'lc-tweaks' );
							...
							$created_label = $created > 0 ? wp_date( 'Y-m-d H:i:s', $created ) : __( 'unknown time', 'lc-tweaks' );
							/* translators: 1: snapshot time, 2: reason, 3: user login, 4: option count */
							$label = sprintf( __( '%1$s | %2$s | by %3$s | %4$d options', 'lc-tweaks' ), $created_label, $reason, $user_login, $option_count );
							?>
					...
					<?php submit_button( __( 'Restore Selected Snapshot', 'lc-tweaks' ), 'dlck-settings-button', 'dlck_restore_snapshot_submit', false ); ?>
				<?php endif; ?>
			</div>
		</div>
	</div>
	...
	<div class="tool-wrap">
		<div class="lc-kit">
			<div class="box-title">
				<h3><?php esc_html_e( 'Export', 'lc-tweaks' ); ?></h3>
				<div class="box-descr">
					<p><?php esc_html_e( 'Choose which settings you’d like to include in the export file.', 'lc-tweaks' ); ?></p>
					<p><?php esc_html_e( 'Customizer options are legacy data from Divi LC Kit and may be empty on newer installs.', 'lc-tweaks' ); ?></p>
				</div>
			</div>
			<div class="box-content minibox" id="dlck_export_box">
				<div>
					...
					<label class="minicheckbox" for="dlck_import_type_settings"><?php esc_html_e( 'LC Tweaks Settings', 'lc-tweaks' ); ?></label>
				</div>
				<div>
					<input type="checkbox" class="minicheckbox" name="dlck_import_type_customizer" id="dlck_import_type_customizer" checked>
					<label class="minicheckbox" for="dlck_import_type_customizer"><?php esc_html_e( 'LC Tweaks Customizer Options (legacy Divi LC Kit)', 'lc-tweaks' ); ?></label>
				...
				<div>
					...
					<?php submit_button( __( 'Export', 'lc-tweaks' ), 'dlck-settings-button', 'dlck_export_submit', false ); ?>
				</div>
			</div>
		</div>
	</div>
	...
	<div class="tool-wrap">
		<div class="lc-kit">
			<div class="box-title">
				<h3><?php esc_html_e( 'Diagnostics', 'lc-tweaks' ); ?></h3>
				<div class="box-descr"><p><?php esc_html_e( 'Download a support diagnostics report with active options, cache status, and key environment details.', 'lc-tweaks' ); ?></p></div>
			</div>
			<div class="box-content minibox" id="dlck_diagnostics_box">
				...
				<?php submit_button( __( 'Download Diagnostics', 'lc-tweaks' ), 'dlck-settings-button', 'dlck_diagnostics_submit', false ); ?>
			</div>
		</div>
	</div>
	...
	<div class="tool-wrap">
		<div class="lc-kit">
			<div class="box-title">
				<h3><?php esc_html_e( 'Import', 'lc-tweaks' ); ?></h3>
				<div class="box-descr"><p><?php esc_html_e( 'Choose the file to import LC Tweaks settings. Export your current settings first if you want a backup.', 'lc-tweaks' ); ?></p></div>
			</div>
			<div class="box-content minibox" id="dlck_import_box">
				...
				<?php submit_button( __( 'Import', 'lc-tweaks' ), 'dlck-settings-button', 'dlck_import_submit', false ); ?>
			</div>
		</div>
	</div>
	...
	<div class="tool-wrap">
		<div class="lc-kit">
			<div class="box-title">
				<h3><?php esc_html_e('CSS/JS Cache', 'lc-tweaks'); ?></h3>
				<div class="box-descr"><p><?php esc_html_e('Remove cached LC Tweaks CSS/JS files generated from active options.', 'lc-tweaks') ?></p></div>
			</div>
			<div class="box-content minibox">
				<button type="button" class="dlck-settings-button" id="dlck-clear-cache">
					<?php esc_html_e( 'Clear LC Tweaks Cache' , 'lc-tweaks') ?>
				</button>
...
<h2 class="tool-section"><?php esc_html_e('Uninstalling the plugin', 'lc-tweaks'); ?></h2>
<div class="tool-wrap">	
	<div class="lc-kit trigger">
		<div class="box-title">
			<h3><?php esc_html_e('Remove Plugin Data', 'lc-tweaks'); ?></h3>	
			
					
			<div class="box-descr"><p><?php esc_html_e("Enable this option if you'd like to remove all custom settings and plugin data when deleting LC Tweaks in the Plugins dashboard.", "lc-tweaks"); ?></p></div>			
		</div>
		<div class="box-content minibox">
			<div class="checkbox">

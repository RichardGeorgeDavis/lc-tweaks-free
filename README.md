...
- **AI Agent Readiness:** serve public content as Markdown for agents, add Content Signals, publish honest discovery headers, optionally exclude WooCommerce products/store pages, enhance included WooCommerce product/catalog Markdown, show readiness diagnostics, and optionally add an Apache `.htaccess` fallback for cached homepage Link headers.
...
- The bundled **Modules** tab and all bundled modules, custom modules, and module overrides, including, but not limited to:
